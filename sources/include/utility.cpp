...
int collisionNotPutArea(std::vector<std::vector<Point>>& notPut, std::vector<Point>& piece){
  //線が交わっているならここではじく
  std::vector<int> hit(static_cast<int>(notPut.size()));
  ...
  //線が交わっていない領域が残っていないならreturn 1
  auto result = std::find(hit.begin(), hit.end(),0);
  if (result == hit.end()) {
	return 1;
  }
		  ...
		  if (notPut[i][k] == piece[j]) {//未設置領域の頂点上にある場合
		   ...
		   //未設置領域の次の頂点上にあるなら次に判定を持ち越す
			if (notPut[i][(k + 1) % notPut[i].size()] != piece[j]) {
			  if (cross(npn, pf) <= 0 && cross(npf, pf) >= 0) {//平行で違う向きなら外延

				Vector nf = notPut[i][(k + 1) % notPut[i].size()] - notPut[i][k];
				if (cross(nf, pf) == 0 && dot(nf, pf) < 0) { 
				  hitflag = 1; break;
				}

				//もし平行のとき、ピースのベクトルが未設置より長いなら外延
				if (cross(npf, pf) == 0 && npf.size() < pf.size()) { 
				  hitflag = 1; break; 
				}
			  }
			  else {//外延
  ...
  result = std::find(hit.begin(), hit.end(), 0);
  if (result == hit.end()) {
	return 1;
  }
